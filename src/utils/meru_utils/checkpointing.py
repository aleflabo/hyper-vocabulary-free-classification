...
import src.utils.meru_utils.distributed as dist
...
class CheckpointManager:
    ...
    def load(self, path: str | Path) -> int:
        ...
        print(f"Rank {rank}: Loading checkpoint from {path}")
        ...
        for key in checkpoint:
            if key in self.checkpointables:
                ...
                print(f"Rank {rank}: Loading {key} from {path}")
        ...
        if len(not_loaded) > 0:
            ...
            print(f"Rank {rank}: Checkpointables not found in file: {not_loaded}")
        return iteration